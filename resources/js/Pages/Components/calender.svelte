        ...
        {#each Array(totalCells) as _, i}
          ...
          {:else}
            ...
            {#if date.getDate() === i - blankCellsBefore + 1 && date.getMonth() === date.getMonth()}
              ...
              <div class="day current" on:click={() => selectDay(i - blankCellsBefore + 1)}>{i - blankCellsBefore + 1}</div>
            {:else if events.find(event => event.date === i - blankCellsBefore + 1 && date.getMonth() === date.getMonth())}
              <!-- Mark the days with events -->
              <div class="day event" on:click={() => selectDay(i - blankCellsBefore + 1)}>{i - blankCellsBefore + 1}</div>
            {:else}
              <div class="day" on:click={() => selectDay(i - blankCellsBefore + 1)}>{i - blankCellsBefore + 1}</div>
            {/if}
          {/if}
        {/each}
      </div>
